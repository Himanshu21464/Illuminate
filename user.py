...
class User:
    ...
    def __view_product__(self):
        ...
        print('|-------------------------CATALOUGE-----------------------|')
        cursor = self.connection.cursor()
        cursor.execute('SELECT "Product name", "Product ID" FROM Product')
    ...
    def __view_brand__(self):
        ...
        print('|-------------------------BRANDS-----------------------|')
        cursor = self.connection.cursor()
        cursor.execute('SELECT "Brand name" FROM Brand')
        result = cursor.fetchall()
        self.print_table(result,cursor)
        cursor.close()
    ...
    def __upgrade__(self,):
        if not self.isauthenticated:
            ...
            return
        print('|-------------------------UPGRADE-----------------------|')
        cursor.execute(f'SELECT "membership" FROM Customer WHERE Customer_ID = {self.uid}')
        ...
        if desired_level!=level:
            cursor.execute(f'UPDATE Customer SET "Membership" = {desired_level} WHERE Customer_ID = {self.uid}')
    ...
    def __add_money__(self):
        ...
        cursor  = self.connection.cursor()
        cursor.execute(f'UPDATE Customer SET "Wallet" = "Wallet" + {input("Enter amount to add: ")} WHERE Customer_ID = {self.uid}')
        self.connection.commit()
        cursor.close()
        print("|+++++Money Added+++++|")
    ...
    def __place_order__(self):
        ...
        cursor = self.connection.cursor()
        cursor.execute(f'SELECT "Cart.Product ID","Cart.Discount","Cart.Quantity","Product.Price" FROM Cart,Product WHERE Customer_ID = {self.uid} AND Cart.Product_ID = Product.Product_ID')
        result = cursor.fetchall()
    ...
    def __view_cart__(self):
        ...
        cursor = self.connection.cursor()
        input('''|-------------Cart------------|''')
        cursor.execute(f'SELECT "Product name", "Product ID" FROM Product WHERE Product_ID IN (SELECT Product_ID FROM Cart WHERE Customer_ID = {self.uid})')
        result = cursor.fetchall()
        result = cursor.fetchall()
        cursor.close()
    ...
    def __view_wishlist__(self):
        ...
        cursor = self.connection.cursor()
        input('''|-------------Cart------------|''')
        cursor.execute(f'SELECT "Product name", "Product ID" FROM Product WHERE Product_ID IN (SELECT Product_ID FROM Wishlist WHERE Customer_ID = {self.uid})')
        result = cursor.fetchall()
        self.print_table(result,cursor)
        cursor.close()
    ...
    def __view_profile__(self):
        if not self.isauthenticated:
            ...
            return
        cursor = self.cursor.connection()
        cursor.execute(f'SELECT * FROM Customer WHERE Customer_ID = {self.uid}')
